...
const Home = () => {
  ...
  useEffect(() => {
    axios
      ...
      .finally(() => {
        setLoading(false); // Hide loading screen after fetching
      });
  }, []);
    ...
    <div>
      ...
      {loading && (
        <div className="fixed inset-0 flex flex-col items-center justify-center bg-white bg-opacity-75 z-50">
          ...
          <p className="mt-4 text-lg font-semibold text-gray-700">Loading articles...</p>
        </div>
      )}
      ...
      {!loading && articles.length === 0 && (
        <div className="flex flex-col items-center justify-center h-screen text-center">
          <h1 className="text-2xl font-bold text-gray-800">No articles available</h1>
          <p className="text-gray-600">Please check back later.</p>
        </div>
      )}

      {/* Main Content (Only shown if articles exist) */}
      {!loading && articles.length > 0 && (
        <>
          <div className="bg-gray-800 text-white text-center py-6 px-4 shadow-lg">
          ...
          <div className="w-[100vw] flex flex-col lg:flex-row justify-center items-start px-6 py-8 bg-gray-50 min-h-screen">
            ...
            <div className="lg:w-[60%] w-full flex flex-col items-center p-4">
              <Link
                ...
                className="relative w-full h-[70vh] rounded-lg overflow-hidden shadow-lg"
              >
                <img
